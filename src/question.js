...
function validateQuestion(question) {
    // 必須項目の確認
    const requiredFields = ["questionId", "imageUrl", "choices", "correctChoiceId"];
    const missingFields = requiredFields.filter(field => !(field in question));
    if (missingFields.length > 0) {
        return { valid: false, message: `必須項目が欠けています: ${missingFields.join(", ")}` };
    }

    // choicesが配列であるか、長さが4であるかを確認
    if (!Array.isArray(question.choices) || question.choices.length !== 4) {
        return { valid: false, message: "'choices' フィールドは配列で、要素が4つである必要があります。" };
    }

    // 各choiceがchoiceIdとtextを持つか確認
    for (const choice of question.choices) {
        if (typeof choice.choiceId !== "number" || typeof choice.text !== "string") {
            return { valid: false, message: "各選択肢は 'choiceId'（数値）と 'text'（文字列）を持つ必要があります。" };
        }
    }
